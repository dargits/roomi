...
const DiscountStatusBadge: React.FC<DiscountStatusBadgeProps> = ({ status }) => {
  const config: Record<string, { label: string; bg: string; text: string; border: string; Icon: React.ComponentType<{ size?: number }> }> = {
    APPLIED: {
      label: 'Đã áp dụng',
      bg: 'bg-green-50',
      text: 'text-green-700',
  ...
  const c = (status && config[status]) || config.APPLIED;
  return (
    <span
...
export interface DiscountPanelProps {
  discount?: InvoiceDiscountResponse | null;
