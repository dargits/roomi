...
import { RoomResponse, RoomStatus } from '../../types';
...
const STATUS_CONFIG: Record<string, { label: string; icon: React.ComponentType<{ size?: number }>; bg: string; border: string; text: string; badge: string }> = {
...
const RoomStatusUpdate: React.FC = () => {
  ...
  const fetchRooms = async () => {
    ...
    try {
      ...
      toastError(err.response?.data?.message || 'Lỗi khi tải danh sách phòng');
    } finally {
      setLoading(false);
    }
  };
  ...
  const floors = useMemo(() => {
    const floorSet = new Set(rooms.map(r => r.floor).filter((f): f is string => Boolean(f)));
  ...
  const displayedRooms = useMemo(() => {
    return rooms.filter(room => {
      const matchSearch = !searchTerm ||
        ...
        (room as any).notes?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="space-y-5">
        ...
        {Object.entries(STATUS_CONFIG).map(([status, cfg]) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-3.5 p-4 rounded-2xl border transition-all duration-150 text-left shadow-sm cursor-pointer ${
                isActive 
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-border-grey shadow-sm">
        ...
        <button
          onClick={fetchRooms}
          disabled={loading}
          className="flex items-center gap-2 px-3.5 py-2 rounded-xl border border-border-grey bg-white hover:bg-surface-container-low transition-colors text-xs font-medium text-on-surface shadow-sm cursor-pointer"
        >
