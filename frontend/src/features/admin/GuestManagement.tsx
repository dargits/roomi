...
const GuestManagement: React.FC = () => {
  ...
  const [selectedGuestHistory, setSelectedGuestHistory] = useState<any[]>([]);
  const [historyLoading, setHistoryLoading] = useState(false);

  const [isLoyaltyModalOpen, setIsLoyaltyModalOpen] = useState(false);
  const [selectedGuestLoyalty, setSelectedGuestLoyalty] = useState<any>(null);
  const [loyaltyLoading, setLoyaltyLoading] = useState(false);

  // NCL-12-CN-005: Modal xóa dữ liệu cá nhân
  const [deleteDataModal, setDeleteDataModal] = useState<{ open: boolean; guest: GuestResponse | null }>({
    open: false,
    guest: null
  });
  ...
  const openEditModal = (guest: GuestResponse) => {
    setFormData({
      id: guest.id,
      name: guest.name,
      phone: guest.phone || '',
  ...
  const viewHistory = async (guest: GuestResponse) => {
  ...
  const viewLoyalty = async (guest: GuestResponse) => {
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-border-grey">
              ...
              <Button
                ...
                onClick={async () => {
                  if (!deleteDataModal.guest) return;
