 ...
 */
const ConcurrencyLogPage: React.FC = () => {
  const { user } = useAuth();
  ...
  const roomOptions = [
    ...
    ...rooms.map(r => ({ value: String(r.id), label: `Phòng ${r.roomNumber} (${r.roomTypeName || ''})` }))
  ];
          ...
          <div className="bg-surface-container-lowest rounded border border-border-grey p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Input
                ...
                onChange={e => setTestForm(p => ({ ...p, requestCount: Number(e.target.value) }))}
              />
              <Input
