...
import { IoAlertCircleOutline, IoBarChartOutline, IoBrushOutline, IoCalendarOutline, IoCheckmarkCircleOutline, IoHappyOutline, IoLogInOutline, IoLogOutOutline, IoPeopleOutline, IoRefreshOutline, IoTrendingUpOutline, IoKeyOutline } from 'react-icons/io5';
...
const StatCard: React.FC<{
  ...
  subLabel?: string;
  onClick?: () => void;
}> = ({ icon: Icon, label, value, color, subLabel, onClick }) => (
  <div
    onClick={onClick}
  ...
  const fetchData = async () => {
    ...
    try {
      const promises: [Promise<any>, Promise<any>, Promise<any>] = [
        isOwnerOrAdmin ? reportApi.getDashboard() : Promise.resolve(null),
        canSeeToday ? reportApi.getTodayCheckInOut() : Promise.resolve([]),
        user?.role === 'HOUSEKEEPER' ? roomApi.getAllRooms('DIRTY') : Promise.resolve([])
      ];
