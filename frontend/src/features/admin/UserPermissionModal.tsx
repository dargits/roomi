...
const UserPermissionModal: React.FC<UserPermissionModalProps> = ({ isOpen, onClose, user }) => {
  ...
  // Form thu hồi quyền
  const [revokingPerm, setRevokingPerm] = useState<any>(null);
  ...
  const handleRevoke = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user?.id || !revokingPerm) return;
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Phân quyền tài khoản: ${user?.name || (user as any)?.account || ''}`}
      maxWidth="max-w-3xl"
    >
        ...
        <div className="p-4 bg-surface-container-low rounded-lg border border-border-grey flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            <div className="text-sm font-semibold text-on-surface">
              {user?.name} <span className="font-normal text-on-surface-variant">(@{(user as any)?.account})</span>
            </div>
          ...
          <>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-sm font-semibold text-on-surface">
                ...
                <span>Quyền mặc định theo vai trò ({(user?.role && roleLabelMap[user.role]) || user?.role})</span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                {overview?.roleDefaultPermissions && overview.roleDefaultPermissions.length > 0 ? (
                  overview.roleDefaultPermissions.map((perm: string, idx: number) => (
                    <div 
                      key={idx}
            ...
            <div className="space-y-3 pt-4 border-t border-border-grey">
                ...
                <form onSubmit={handleGrant} className="p-4 bg-surface-container-lowest border-2 border-indigo-200 rounded-lg space-y-4 animate-fade-in">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        className="w-full text-xs p-2.5 border border-border-grey rounded focus:outline-none focus:border-primary bg-white text-on-surface"
                        value={selectedPermission}
                        onChange={(e) => setSelectedPermission(e.target.value)}
                        required
                      >
                        <option value="">-- Chọn quyền xem --</option>
                        {overview?.availableExtraPermissions?.map((perm: string) => (
              ...
              <div className="space-y-2">
                {overview?.extraPermissions && overview.extraPermissions.length > 0 ? (
                  overview.extraPermissions.map((item: any) => (
                    <div
                      key={item.id}
            ...
            {revokingPerm && (
              <div className="p-4 bg-red-50 border border-red-300 rounded-lg space-y-3 animate-fade-in">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    className="w-full text-xs p-2 border border-red-200 rounded focus:outline-none focus:border-red-500 bg-white text-on-surface"
