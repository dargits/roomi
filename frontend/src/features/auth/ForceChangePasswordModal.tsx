...
const ForceChangePasswordModal: React.FC<ForceChangePasswordModalProps> = ({ isOpen, account, onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!account) {
      toastError('Không tìm thấy thông tin tài khoản.');
      return;
    }
    if (!tempPassword) {
    ...
    try {
      ...
      console.error(err);
      toastError(err.response?.data?.message || err.message || 'Có lỗi xảy ra khi đổi mật khẩu.');
    } finally {
      setLoading(false);
    }
  };
