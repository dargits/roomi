...
import { OccupancyReportResponse, OccupancyReportRow } from '../../types';
 ...
 */
const OccupancyVisualChart: React.FC<{ rows: OccupancyReportRow[]; overallRate: number }> = ({ rows }) => {
                ...
                >
                  ...
                  {isHovered && (
                    <div className="absolute bottom-full mb-3 z-30 flex flex-col items-center pointer-events-none animate-in fade-in zoom-in-95 duration-150">
                      <div className="bg-[#002146] text-white text-xs rounded-xl py-2 px-3 shadow-xl whitespace-nowrap text-center border border-blue-900">
                        <p className="font-semibold text-blue-200 border-b border-blue-900 pb-1 mb-1">{dateLabel}</p>
                        <p className="font-bold text-emerald-400 text-sm">{rate.toFixed(1)}%</p>
                        <p className="text-[11px] text-slate-400 mt-0.5">{row.occupiedRooms || 0} / {row.availableRooms || 0} phòng có khách</p>
                      </div>
                      <div className="w-2 h-2 bg-[#002146] rotate-45 -mt-1" />
                    </div>
                  )}
...
const SummaryCard: React.FC<{ label: string; value?: string | number | null; sub?: string; color?: string }> = ({
  label,
  value,
  sub,
...
const OccupancyReport: React.FC = () => {
  ...
  const [data,     setData]     = useState<OccupancyReportResponse | null>(null);
  ...
  const exportCSV = () => {
    ...
    const csvContent = [
      headers.join(','),
      ...rows.map(r => [r.date, r.totalRooms || totalRooms, r.occupiedRooms, (parseRate(r.occupancyRate)).toFixed(2)].join(','))
    ].join('\n');
