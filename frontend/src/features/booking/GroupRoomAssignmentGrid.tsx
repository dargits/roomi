...
interface GroupAssignmentSuggestion {
  assignments: AssignmentLine[];
}

interface GroupRoomAssignmentGridProps {
  group?: any;
  suggestion: GroupAssignmentSuggestion | null;
  selections: Record<number, string>;
  onUpdateSelection: (bookingId: number, roomId: string) => void;
...
const GroupRoomAssignmentGrid: React.FC<GroupRoomAssignmentGridProps> = ({
  suggestion,
  selections,
  onUpdateSelection,
  onApplyAllSuggested,
  ...
  const allAvailableRoomsMap = useMemo(() => {
    const map = new Map<number, RoomResponse>();
  ...
  const selectedRoomIds = useMemo(() => {
    ...
    Object.entries(selections).forEach(([, rId]) => {
