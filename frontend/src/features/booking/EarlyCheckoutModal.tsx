...
const EarlyCheckoutModal: React.FC<EarlyCheckoutModalProps> = ({ isOpen, onClose, bookingId, guestName, onSuccess }) => {
  const { success: toastSuccess } = useToast();
  const [loadingPreview, setLoadingPreview] = useState<boolean>(true);
  const [previewData, setPreviewData] = useState<EarlyCheckoutPreview | null>(null);
        ...
        {loadingPreview ? (
          ...
          <div className="space-y-4 text-sm">
            <div className="p-4 bg-amber-50 border border-amber-200 text-amber-900 rounded-lg flex items-start gap-2.5">
              ...
              <div>
                <div className="font-semibold">Khách {guestName ? `"${guestName}"` : ''} trả phòng trước ngày dự kiến</div>
