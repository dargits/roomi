...
import { PublicGroupBookingRequestResponse } from '../../types';
...
interface ModalState {
  ...
  request: PublicGroupBookingRequestResponse | null;
  reason: string;
}

const PublicGroupBookingRequestList: React.FC = () => {
  const { toastSuccess } = useToast();
  const [requests, setRequests] = useState<PublicGroupBookingRequestResponse[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [processing, setProcessing] = useState<boolean>(false);
  const [depositGroup, setDepositGroup] = useState<any>(null);
  ...
  const openApproveModal = (request: PublicGroupBookingRequestResponse) => {
    setErrorMsg('');
    setModalState({
      isOpen: true,
      type: 'APPROVE',
  ...
  const openRejectModal = (request: PublicGroupBookingRequestResponse) => {
    setErrorMsg('');
    setModalState({
      isOpen: true,
      type: 'REJECT',
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (type === 'APPROVE') {
        const approvedReq: any = await publicGroupBookingRequestApi.approve(request.id);
  ...
  return (
    <section className="border-t-8 border-surface-container-low bg-surface-container-lowest p-6">
      ...
      <div className="flex flex-col min-h-[580px] justify-between">
        <div className="overflow-x-auto rounded-lg border border-border-grey flex-1">
