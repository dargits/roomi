...
const PendingDepositList: React.FC = () => {
  ...
  const filteredDeposits = deposits.filter((d) => {
    const matchesSearch = 
      ...
      ((d as any).groupBookingId && String((d as any).groupBookingId).includes(searchTerm)) ||
                ...
                {paginatedDeposits.map((d) => (
                  <tr key={d.id} className="hover:bg-surface-container-low/40 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      ...
                      ) : (d as any).groupBookingId ? (
                        <button
                          type="button"
                          onClick={() => navigate(`/manage/bookings?tab=groups&groupId=${(d as any).groupBookingId}`)}
                          className="font-semibold text-purple-700 hover:underline inline-flex items-center gap-1 text-left cursor-pointer"
                        >
                          <IoPeopleOutline size={14} className="shrink-0 text-purple-600" />
                          <span>Đoàn #{(d as any).groupBookingId}</span>
                        </button>
                      ) : (
                        <button
                          type="button"
                    ...
                    <td className="py-3 px-4 text-right">
                      {d.bookingId ? (
                        ...
                        <Button
                          ...
                          variant="ghost"
                          icon={IoEyeOutline}
                          onClick={() => navigate(`/manage/bookings?tab=groups${(d as any).groupBookingId ? `&groupId=${(d as any).groupBookingId}` : ''}`)}
                        >
                          Xem chi tiết
                        </Button>
                      )}
