...
const BookingInvoiceTab: React.FC<BookingInvoiceTabProps> = ({ bookingId, status, booking, onPrintInvoice }) => {
  ...
  const [provisionalServices, setProvisionalServices] = useState<any[]>([]);
  ...
  const openPaymentForm = (remAmount: number) => {
    const defaultAmt = remAmount > 0 ? String(remAmount) : '';
    setNewPayment({
      amount: defaultAmt,
      paymentMethod: 'CASH',
  ...
  const handleMethodChange = (method: string) => {
    ...
    setNewPayment(prev => ({
      ...
      amount: targetAmt > 0 ? String(targetAmt) : '',
      note: defaultNote
    }));
    if (method === 'CASH') {
      setReceivedCash(targetAmt > 0 ? String(targetAmt) : '');
    }
  };

  const handleSetAmount = (amt: number, remAmount: number) => {
    const validAmt = Math.min(Math.max(0, amt), remAmount);
    setNewPayment(prev => ({ ...prev, amount: validAmt > 0 ? String(validAmt) : '' }));
    if (newPayment.paymentMethod === 'CASH') {
      setReceivedCash(validAmt > 0 ? String(validAmt) : '');
    }
  };
  ...
  const getCashPresets = (payAmount: number) => {
  ...
  useEffect(() => {
    if (remainingAmount > 0) {
      ...
      setReceivedCash(String(remainingAmount));
    } else {
      setNewPayment(prev => ({
        ...prev,
        amount: ''
