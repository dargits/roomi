...
interface ExtendAvailabilityResponse {
  available: boolean;
...
const ExtendStayModal: React.FC<ExtendStayModalProps> = ({ isOpen, onClose, bookingId, booking, onSuccess }) => {
  const { success: toastSuccess } = useToast();
  const [nights, setNights] = useState<string>('1');
  const [availability, setAvailability] = useState<ExtendAvailabilityResponse | null>(null);
  ...
  const checkNights = async (numNights: string) => {
          ...
          <div className="space-y-3">
            {availability.available ? (
              <div className="space-y-3">
                ...
                {availability.nightPrices && availability.nightPrices.length > 0 && (
