...
const StayDeclarationPage: React.FC = () => {
  const { user } = useAuth();
  const { success, error: toastError } = useToast();

  const userRole = user?.role || '';
  const hasAccess = ['OWNER', 'RECEPTIONIST', 'ADMIN'].includes(userRole);
  const canViewFullId = CAN_VIEW_FULL_ID.includes(userRole);
  const canComplete = ['OWNER', 'RECEPTIONIST', 'ADMIN'].includes(userRole);

  // Tab State: 'today' | 'history'
  const [searchParams, setSearchParams] = useSearchParams();
  const activeTab = searchParams.get('tab') || 'today';
  const setActiveTab = (newTab: string) => setSearchParams({ tab: newTab }, { replace: true });
            ...
            {uploadData.documentType === 'NATIONAL_ID' ? (
              <div className="grid grid-cols-2 gap-6 mt-4">
                ...
                <div className="flex flex-col items-center">
                      ...
                      <>
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            const input = document.getElementById('frontFileInput') as HTMLInputElement | null;
                  ...
                  <div className="mt-3 w-full flex justify-between items-center px-1">
                    ...
                    <Button variant="outline" size="sm" icon={IoCloudUploadOutline} onClick={() => (document.getElementById('frontFileInput') as HTMLInputElement | null)?.click()}>
                      Tải lên
                    </Button>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center">
                  <div className="relative w-full aspect-video border-2 border-dashed border-border-grey rounded-xl overflow-hidden bg-surface-container-low hover:bg-surface-container transition-colors flex flex-col items-center justify-center group cursor-pointer"
                       onClick={() => (document.getElementById('backFileInput') as HTMLInputElement | null)?.click()}
                  >
                      ...
                      <>
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            const input = document.getElementById('backFileInput') as HTMLInputElement | null;
                            if (input) input.value = '';
                          }}
                          className="absolute top-2 right-2 bg-black/50 text-white rounded-full p-1 hover:bg-black/70 transition-colors z-10"
                          title="Xóa ảnh này"
                  ...
                  <div className="mt-3 w-full flex justify-between items-center px-1">
                    ...
                    <Button variant="outline" size="sm" icon={IoCloudUploadOutline} onClick={() => (document.getElementById('backFileInput') as HTMLInputElement | null)?.click()}>
                      Tải lên
                    </Button>
                  </div>
                </div>
              ...
              <div className="flex flex-col items-center mt-4">
                <div className="relative w-full max-w-md aspect-video border-2 border-dashed border-border-grey rounded-xl overflow-hidden bg-surface-container-low hover:bg-surface-container transition-colors flex flex-col items-center justify-center group cursor-pointer"
                     onClick={() => (document.getElementById('passportFileInput') as HTMLInputElement | null)?.click()}
                >
                  {uploadData.frontPreview ? (
                    <>
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setUploadData({ ...uploadData, frontFile: null, frontPreview: null });
                          const input = document.getElementById('passportFileInput') as HTMLInputElement | null;
                          if (input) input.value = '';
                        }}
                        className="absolute top-2 right-2 bg-black/50 text-white rounded-full p-1 hover:bg-black/70 transition-colors z-10"
                        title="Xóa ảnh này"
                ...
                <div className="mt-3 w-full max-w-md flex justify-between items-center px-1">
                  ...
                  <Button variant="outline" size="sm" icon={IoCloudUploadOutline} onClick={() => (document.getElementById('passportFileInput') as HTMLInputElement | null)?.click()}>
                    Tải lên
                  </Button>
                </div>
              </div>
