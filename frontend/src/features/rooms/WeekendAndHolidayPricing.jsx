...
const WeekendAndHolidayPricing = ({ roomTypeId, roomTypeName, basePrice }) => {
  const { toastSuccess, toastError, toastWarning } = useToast();
  const confirm = useConfirm();
  const [activeTab, setActiveTab] = useState('weekend'); // 'weekend' | 'holiday'
  const [loading, setLoading] = useState(false);

  // Weekend state
  const [weekendPrice, setWeekendPrice] = useState(basePrice || 0);
  const [selectedDays, setSelectedDays] = useState(['FRIDAY', 'SATURDAY', 'SUNDAY']);
  const [weekendConfigId, setWeekendConfigId] = useState(null);
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        if (first.weekendDays) {
          setSelectedDays(first.weekendDays.split(',').map(d => d.trim()));
        }
      } else {
  ...
  return (
    <div className="bg-surface rounded-xl border border-border-grey p-4 space-y-4 text-sm mt-2">
      ...
      {/* Thông báo quy tắc ưu tiên giá (NCL-02-CN-006) */}
          ...
          <form onSubmit={handleAddHoliday} className="grid grid-cols-1 sm:grid-cols-12 gap-2.5 p-3 bg-surface-container-low rounded-xl border border-border-grey/60 items-end">
            ...
            <div className="sm:col-span-3">
              <Input
                ...
                onChange={(e) => setNewHoliday({ ...newHoliday, pricePerNight: e.target.value })}
                required
              />
            </div>
