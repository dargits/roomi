...
const PublicGroupBookingModal: React.FC<PublicGroupBookingModalProps> = ({ 
  isOpen, 
  onClose, 
  roomTypes, 
  initialRoom, 
  ...
  const updateRoom = (index: number, field: keyof RoomLine, value: any) => {
  ...
  if (success) {
    ...
    const nights = formData.checkInDate && formData.checkOutDate 
      ? Math.max(1, Math.round((new Date(formData.checkOutDate).getTime() - new Date(formData.checkInDate).getTime()) / (1000 * 60 * 60 * 24)))
      : 1;
